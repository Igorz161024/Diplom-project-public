...
### Зміни у роутерах
- Видалено параметр `prefix` у всіх файлах роутерів:
  - journal_router.py
  - finance_router.py
  - inventory_router.py
  - purchases_router.py
  - sales_router.py
  - legal_router.py
- Залишено лише `tags=[...]` для групування ендпоінтів у Swagger UI.
- Код роутерів тепер містить тільки CRUD‑операції та бізнес‑логіку без дублювання префіксів.

### Зміни у main.py
- Централізовано додано префікси при підключенні роутерів:
  - `/api/journal`
  - `/api/finance`
  - `/api/inventory`
  - `/api/purchases`
...
### Підготовка інсталяції для третіх осіб
- Створено файл `start-erp.ps1` для автоматичного запуску ERP‑системи на Windows (бекенд, фронтенд, база даних).
- Налаштовано `docker-compose.yml` для підняття контейнера PostgreSQL (`erp_db`) з volume‑mount, щоб сторонні користувачі могли швидко розгорнути базу.
...
- Підготовлено інструкції для третіх осіб:
  1. Клонувати репозиторій з GitHub.
  2. Запустити Docker Desktop.
  3. Виконати `start-erp.ps1` для автоматичного старту проєкту.
  4. Відкрити фронтенд у браузері (`http://localhost:3000`) та бекенд API (`http://localhost:8000`).
- Мета: забезпечити можливість стороннім користувачам встановити та запустити ERP‑систему на власному ПК без додаткових налаштувань.

### Мета рефакторингу та інсталяції
...
- Чистота коду → роутери відповідають тільки за CRUD‑операції, без зайвих параметрів.
- Зручність тестування → всі модулі доступні через єдину схему URL.
...
### Додано
- Файл `start-erp.sh` для автоматичного запуску ERP у Linux/WSL середовищі.
...
- Блок документації API у `README-deploy.md` з посиланням на Swagger (http://localhost:8000/docs) та переліком основних модулів.

### Змінено
- Оновлено `README-deploy.md`: тепер він містить єдиний цілісний текст для користувачів, без поділу на приватний/публічний.
- Замінено всі згадки “для викладача” на **“Інструкція для користувачів”**.

### Перевірено
- Запуск ERP через `start-erp.ps1` (Windows) та `start-erp.sh` (Linux/WSL).
- Відновлення бази даних з дампу працює коректно.
- Swagger API доступний за адресою http://localhost:8000/docs.
